    ...
    function getBookOffers() {
      $.get("/books/offered", function (data) {
        ...
        if (!book_requests || !book_requests.length) {
          ...
          displayOffers(book_requests, user_id);
        }
      });
    }
  
    function displayOffers(results_list, id) {
      ...
      for (i=0; i < results_list.length; i++) {
        ...
        zipCode.text("Address: " + results_list[i].User.preferredDropAddress)
        ...
        requestLink.text("Want this book? Click to notify the owner!");
